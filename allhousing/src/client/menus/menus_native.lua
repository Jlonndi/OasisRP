...
NativeWardrobeMenu = function(d)
  ...
  if (#outfits > 0) then
    for i,label in pairs(outfits) do
      ...
      outfit.Activated = function(...) NativeOutfitMenu(d,i,label); end
...
NativeOutfitMenu = function(d,index,label)
  _Pool:CloseAllMenus()

  local outfitMenu = CreateNativeUIMenu("Outfit "..index,label)
  local equip = NativeUI.CreateItem(Labels['EquipOutfit'],"")
  local delete = NativeUI.CreateItem(Labels['DeleteOutfit'],"")
  
  equip.Activated = function(...) 
    SetOutfit(index,label)
  end
  outfitMenu:AddItem(equip)

  delete.Activated = function(...) 
    _Pool:CloseAllMenus()
    DeleteOutfit(index,label)
    NativeWardrobeMenu(d)
  end
  outfitMenu:AddItem(delete)

  outfitMenu:RefreshIndex()
...
NativeGarageOwnedMenu = function(d)
  ...
  if IsPedInAnyVehicle(ped,false) then
    ...
    while not HasAnimDictLoaded("mini@safe_cracking") do RequestAnimDict("mini@safe_cracking"); Citizen.Wait(5); end
