		...
		if DoesEntityExist(scrap_NPC) then 
			...
			if GetDistanceBetweenCoords(coords, NPC_coords.x, NPC_coords.y, NPC_coords.z, false) < 2.0 and not interacting then
				...
				if IsControlJustPressed(0, 38) and not interacting then
					ESX.TriggerServerCallback('oasis_junkyard:getCopsCount', function(copsOnline)
						if copsOnline then
							ESX.TriggerServerCallback('oasis_junkyard:getCarListTimer', function(cooldown)
			...
			if distance < 2.0 and not scrappingCar then
				if isInsideScrapCar(carHash) then
					...
					if IsControlJustPressed(0, 38) then
						...
						ESX.TriggerServerCallback('oasis_junkyard:isVehicleOwned', function(vehOwned)
						...
						ESX.TriggerServerCallback('oasis_junkyard:isVehicleOwned', function(vehOwned)
							if vehOwned then
								if Config.DeleteOwnedVehicles then 
									Citizen.Wait(15000)
									TriggerServerEvent('oasis_junkyard:deleteOwnedVehicle', plate)
								end
							end
							...
							TriggerServerEvent('oasis_junkyard:getPayment', carFinal, round(GetEntityHealth(scrapCar)/10, 0))
...
RegisterNetEvent('oasis_junkyard:lockpickCL')
AddEventHandler('oasis_junkyard:lockpickCL',function(k,v)
	...
	if DoesEntityExist(vehicle) then
		if GetDistanceBetweenCoords(coords, veh_coords.x, veh_coords.y, veh_coords.z, true) < 2.0 then
			if GetVehicleDoorLockStatus(vehicle) ~= 2 or not GetVehicleDoorsLockedForPlayer(vehicle) then
				ESX.TriggerServerCallback('oasis_junkyard:removeLockpick', function(lockpickRemoved)
