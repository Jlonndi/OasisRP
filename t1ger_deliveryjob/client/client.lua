							...
							if level >= data.current.level then 
								if data.current.certificate == false then 
									if #selected < Config.ShopsDeliveryLimit  then 
										local dup = false
										for i = 1, #selected do if selected[i].orderData.id == data.current.orderData.id or selected[i].orderData.shopID ~= data.current.orderData.shopID then dup = true break end end
...
function SetShopRoute(jobValue, orderData)
	local order = orderData[1].data
	deliveryPos = order.pos 
	SetDeliveryBlip(deliveryPos[1], deliveryPos[2], deliveryPos[3])
	vehBodyHealth = GetVehicleBodyHealth(jobVehicle)
	for i = 1, #orderData do
		parcelPrice = parcelPrice + calculatePrice(jobValue)
	end
end
