...
Citizen.CreateThread(function()
	...
	Citizen.Wait(1000)
    TriggerServerEvent('oasismbrp:getVehicles')
    --GetJob()
end)
    ...
    while true do
        ...
        if (inRing) then
            ...
            if IsControlJustReleased(1, 38) then --38 = E Key
               TriggerServerEvent('oasismbrp:RingRing')
            end
        end
...
---- Client Events ----
RegisterNetEvent('oasismbrp:ShowMSG')
AddEventHandler('oasismbrp:ShowMSG', function(msg, OnlyForJob)
...
RegisterNetEvent('oasismbrp:ResultSocietyMoney')
AddEventHandler('oasismbrp:ResultSocietyMoney', function(money)
    currentSocietyMoney = money
    gotDataMoney = true
end)

RegisterNetEvent('oasismbrp:getEmployeesResult')
AddEventHandler('oasismbrp:getEmployeesResult', function(employee)
    employees = {}
    employees = employee
    gotEmployees = true
end)
...
RegisterNetEvent('oasismbrp:ShowAdvancedMSG')
AddEventHandler('oasismbrp:ShowAdvancedMSG', function(pic, title, subtitle, msg)
    ShowAdvancedNotification(pic, title, subtitle, msg)
end)

RegisterNetEvent('oasismbrp:getVehiclesResult')
AddEventHandler('oasismbrp:getVehiclesResult', function(Data)
    Vehicles = {}
    Vehicles = Data
    GetJob()
end)

RegisterNetEvent('oasismbrp:CheckTimeResult')
AddEventHandler('oasismbrp:CheckTimeResult', function(data)
    timeToLiver = data
end)
---- functions ----
...
function GenerateBossMenu()
    ...
    TriggerServerEvent('oasismbrp:getSocietyMoney')
    TriggerServerEvent('oasismbrp:getEmployeesDB')
		...
		local onScreenDisplayResult_shopmenu_amount = CreateDialog(Translation[Cfg.Locale]['how_much'])
			if (onScreenDisplayResult_shopmenu_amount ~= nil) then
				if tonumber(onScreenDisplayResult_shopmenu_amount) then
                    ...
                    TriggerServerEvent('oasismbrp:addSocietyMoney', amount)
    ...
    for j, ply in pairs(players) do
        ...
        hirePlayerItem.Activated = function(sender, index)
            TriggerServerEvent('oasismbrp:hire', GetPlayerServerId(ply), GetPlayerName(ply))
        end
    end
    ...
    for i, emp in pairs(employees) do
        ...
        for o, grade in pairs(Cfg.Grades) do
            ...
            grade_item.Activated = function(sender, item)
                TriggerServerEvent('oasismbrp:promote', emp.identifier, emp.firstname, emp.lastname, grade.number, grade.label)
            end
        end
        fire.Activated = function(sender, item)
            TriggerServerEvent('oasismbrp:fire', emp.identifier, emp.firstname, emp.lastname)
            TriggerServerEvent('oasismbrp:getEmployeesDB')
        end
    end
...
function GenerateComputerMenu(x, y, z, rot)
    ...
    if Cfg.useOneSyncInfinity then
        ESX.TriggerServerCallback('oasismbrp:getPlayersInArea', function(playersInArea_res)
                ...
                if v.storage > 0 then
                    ...
                    if tonumber(price) then
                        ...
                        if price ~= nil then
                            ...
                            -- MONEY Check
                            ESX.TriggerServerCallback('oasismbrp:hasEnoughMoney', function(hasEnoughMoney)
                                
                                if hasEnoughMoney then
                                    local veh = SpawnVehicle(v.model, SpawnLocation.x, SpawnLocation.y, SpawnLocation.z, SpawnLocation.rot, false, false, false)
                                    --print('SpawnVehicle')
                                    TriggerServerEvent('oasismbrp:removeMoney', ServerIdtargetPlayer, price)
                                    ...
                                    TriggerServerEvent('oasismbrp:BuyVeh', ServerIdtargetPlayer, v.manufacturer, v.label, vehProps, v.storage, v.model, price, NetworkGetNetworkIdFromEntity(veh), GetEntityCoords(veh), GetEntityHeading(veh))
    ...
    for k,v in pairs(Vehicles) do
        ...
        ps_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasismbrp:updateInfo', v.id, 'ps', newValue)
            end
        end
        ...
        maxSpeed_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasismbrp:updateInfo', v.id, 'maxSpeed', newValue)
            end
        end
        ...
        speedup.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasismbrp:updateInfo', v.id, 'speedUp', newValue)
            end
        end
        ...
        modifikation.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasismbrp:updateInfo', v.id, 'modification', newValue)
            end
        end
        ...
        tankcapacity.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasismbrp:updateInfo', v.id, 'fuelcapacity', newValue)
            end
        end
        ...
        if isBoss then
            ...
            liver.Activated = function(sender, item)
                ...
                if tonumber(ordertotal) then
                    ...
                    TriggerServerEvent('oasismbrp:liverVehicle', v.model, ordertotal, v.label, v.manufacturer)
                end
            end
            ...
            produce.Activated = function(sender, item)
                ...
                if tonumber(producetotal) then
                    ...
                    TriggerServerEvent('oasismbrp:produceVehicle', v.model, producetotal, v.label, v.manufacturer)
                end
            end
            if v.orderd == 1 then
        ...
        for k,v in pairs(Vehicles) do
            if v.produced == 1 then
                ...
                check.Activated = function(sender, item)
                    TriggerServerEvent('oasismbrp:CheckTime', v.time, v.lastorder, v.model, v.storage, v.producetotal)
                end
            end
        end
    end
...
RegisterCommand("Clientmbrp", function(source, args, rawCommand)
    TriggerServerEvent('oasismbrp:getVehicles')
end, false)
