	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'mechanic_actions', {
		...
		if data.current.value == 'vehicle_list' then
			if Config.EnableSocietyOwnedVehicles then
				...
				local elements = {
					{label = 'Cat Flatbed', value = 'ct660tow'},
					{label = 'G Tow Flatbed', value = 'gtow'},
					{label = 'Ram Flatbed', value = 'bc205500w'}
				}
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 166) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
