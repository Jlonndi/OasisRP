...
Citizen.CreateThread(function()
	...
	Citizen.Wait(1000)
    TriggerServerEvent('oasislaynz:getVehicles')
    --GetJob()
end)
    ...
    while true do
        ...
        if (inRing) then
            ...
            if IsControlJustReleased(1, 38) then --38 = E Key
               TriggerServerEvent('oasislaynz:RingRing')
            end
        end
...
---- Client Events ----
RegisterNetEvent('oasislaynz:ShowMSG')
AddEventHandler('oasislaynz:ShowMSG', function(msg, OnlyForJob)
...
RegisterNetEvent('oasislaynz:ResultSocietyMoney')
AddEventHandler('oasislaynz:ResultSocietyMoney', function(money)
    currentSocietyMoney = money
    gotDataMoney = true
end)

RegisterNetEvent('oasislaynz:getEmployeesResult')
AddEventHandler('oasislaynz:getEmployeesResult', function(employee)
    employees = {}
    employees = employee
    gotEmployees = true
end)
...
RegisterNetEvent('oasislaynz:ShowAdvancedMSG')
AddEventHandler('oasislaynz:ShowAdvancedMSG', function(pic, title, subtitle, msg)
    ShowAdvancedNotification(pic, title, subtitle, msg)
end)

RegisterNetEvent('oasislaynz:getVehiclesResult')
AddEventHandler('oasislaynz:getVehiclesResult', function(Data)
    Vehicles = {}
    Vehicles = Data
    GetJob()
end)

RegisterNetEvent('oasislaynz:CheckTimeResult')
AddEventHandler('oasislaynz:CheckTimeResult', function(data)
    timeToLiver = data
end)
---- functions ----
...
function GenerateBossMenu()
    ...
    TriggerServerEvent('oasislaynz:getSocietyMoney')
    TriggerServerEvent('oasislaynz:getEmployeesDB')
		...
		local onScreenDisplayResult_shopmenu_amount = CreateDialog(Translation[Cfg.Locale]['how_much'])
			if (onScreenDisplayResult_shopmenu_amount ~= nil) then
				if tonumber(onScreenDisplayResult_shopmenu_amount) then
                    ...
                    TriggerServerEvent('oasislaynz:addSocietyMoney', amount)
    ...
    for j, ply in pairs(players) do
        ...
        hirePlayerItem.Activated = function(sender, index)
            TriggerServerEvent('oasislaynz:hire', GetPlayerServerId(ply), GetPlayerName(ply))
        end
    end
    ...
    for i, emp in pairs(employees) do
        ...
        for o, grade in pairs(Cfg.Grades) do
            ...
            grade_item.Activated = function(sender, item)
                TriggerServerEvent('oasislaynz:promote', emp.identifier, emp.firstname, emp.lastname, grade.number, grade.label)
            end
        end
        fire.Activated = function(sender, item)
            TriggerServerEvent('oasislaynz:fire', emp.identifier, emp.firstname, emp.lastname)
            TriggerServerEvent('oasislaynz:getEmployeesDB')
        end
    end
...
function GenerateComputerMenu(x, y, z, rot)
    ...
    if Cfg.useOneSyncInfinity then
        ESX.TriggerServerCallback('oasislaynz:getPlayersInArea', function(playersInArea_res)
                ...
                if v.storage > 0 then
                    ...
                    if tonumber(price) then
                        ...
                        if price ~= nil then
                            ...
                            -- MONEY Check
                            ESX.TriggerServerCallback('oasislaynz:hasEnoughMoney', function(hasEnoughMoney)
                                
                                if hasEnoughMoney then
                                    local veh = SpawnVehicle(v.model, SpawnLocation.x, SpawnLocation.y, SpawnLocation.z, SpawnLocation.rot, false, false, false)
                                    --print('SpawnVehicle')
                                    TriggerServerEvent('oasislaynz:removeMoney', ServerIdtargetPlayer, price)
                                    ...
                                    TriggerServerEvent('oasislaynz:BuyVeh', ServerIdtargetPlayer, v.manufacturer, v.label, vehProps, v.storage, v.model, price, NetworkGetNetworkIdFromEntity(veh), GetEntityCoords(veh), GetEntityHeading(veh))
    ...
    for k,v in pairs(Vehicles) do
        ...
        ps_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasislaynz:updateInfo', v.id, 'ps', newValue)
            end
        end
        ...
        maxSpeed_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasislaynz:updateInfo', v.id, 'maxSpeed', newValue)
            end
        end
        ...
        speedup.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasislaynz:updateInfo', v.id, 'speedUp', newValue)
            end
        end
        ...
        modifikation.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasislaynz:updateInfo', v.id, 'modification', newValue)
            end
        end
        ...
        tankcapacity.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasislaynz:updateInfo', v.id, 'fuelcapacity', newValue)
            end
        end
        ...
        if isBoss then
            ...
            liver.Activated = function(sender, item)
                ...
                if tonumber(ordertotal) then
                    ...
                    TriggerServerEvent('oasislaynz:liverVehicle', v.model, ordertotal, v.label, v.manufacturer)
                end
            end
            ...
            produce.Activated = function(sender, item)
                ...
                if tonumber(producetotal) then
                    ...
                    TriggerServerEvent('oasislaynz:produceVehicle', v.model, producetotal, v.label, v.manufacturer)
                end
            end
            if v.orderd == 1 then
        ...
        for k,v in pairs(Vehicles) do
            if v.produced == 1 then
                ...
                check.Activated = function(sender, item)
                    TriggerServerEvent('oasislaynz:CheckTime', v.time, v.lastorder, v.model, v.storage, v.producetotal)
                end
            end
        end
    end
