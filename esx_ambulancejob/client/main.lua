...
Citizen.CreateThread(function()
	while true do
		...
		if IsDead then
			...
			EnableControlAction(0,249,true) -- disable N stop speaking
...
function OnPlayerDeath()
	...
	ClearPedTasksImmediately(plypd)
	...
	Citizen.CreateThread(function()
		...
		while timer > 0 and IsDead do
			...
			if GetLastInputMethod(2) then
				if IsControlPressed(2, Keys['G']) then
					SendDistressSignal()
...
function SendDistressSignal()
	...
	exports['core_dispach']:addCall("HELP", "Person Incompacitated", {
		{icon="fa-ambulance", info="Non responsive person"},
		}, {pos.x, pos.y, pos.z}, "ambulance", 3000, 11, 5 )
...
function StartDeathTimer()
	...
	Citizen.CreateThread(function()
		...
		while bleedoutTimer > 0 and IsDead do
			...
			if not Config.EarlyRespawnFine then
				...
				if GetLastInputMethod(2) then
					...
					if IsControlPressed(2, Keys['E']) and timeHeld > 60 then
						RemoveItemsAfterRPDeath()
						break
					end
		...
		if GetLastInputMethod(2) then
			if IsControlPressed(2, Keys['E']) then
				timeHeld = timeHeld + 1
			else
				timeHeld = 0
			end
...
AddEventHandler('esx_ambulancejob:revive', function()
	...
	TriggerServerEvent('esx_ambulancejob:setDeathStatus', false)
	TriggerServerEvent("skeletalsystem:HealBones",source,"all")
