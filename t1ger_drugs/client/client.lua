...
AddEventHandler('t1ger_drugs:itemUseCL',function()
	...
	if is_allowed then 
		...
		ShowNotifyESX("Cops & Police is not allowed!")
	end
end)
		...
		if #inv_drugs > 0 then
			ESX.TriggerServerCallback('t1ger_drugs:getPlayerMaxCap', function(maxCap)
				if not maxCap then
					...
					ShowNotifyESX(Lang['max_cap_reached'])
					selling_drugs = false
				end
			end)
		else
			ShowNotifyESX(Lang['no_more_drugs'])
