...
AddEventHandler('esx_extraitems:bulletproof', function()
	...
	AddArmourToPed(playerPed, 130)
	SetPedArmour(playerPed, 130)
end)
...
AddEventHandler('esx_extraitems:drill', function(source)
	...
	if IsPedSittingInAnyVehicle(playerPed) then
		...
		exports.pNotify:SendNotification({text = _U('error_veh'), type = "info", timeout = 5000})
	else
		if IsPedOnFoot(playerPed) then
			...
			exports.pNotify:SendNotification({text = _U('error_no_foot'), type = "info", timeout = 5000})
		end
	end
end)
-- End of Drill
...
AddEventHandler('esx_extraitems:firstaidkit', function()
	...
	if health > 0 and health < max then
		...
		exports.pNotify:SendNotification({text = _U('used_firstaidkit'), type = "info", timeout = 5000})
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				if chance <= 66 then
					...
					exports.pNotify:SendNotification({text = _U('veh_unlocked'), type = "info", timeout = 5000})
				else
					--ESX.ShowNotification(_U('hijack_failed'))
					exports.pNotify:SendNotification({text = _U('hijack_failed'), type = "info", timeout = 5000})
					ClearPedTasksImmediately(playerPed)
				end
			end)
		end
...
AddEventHandler('esx_extraitems:oxygenmask', function()
	...
	ESX.Game.SpawnObject('p_s_scuba_mask_s', {
		...
		ESX.Game.SpawnObject('p_s_scuba_tank_s', {
			...
			--exports.pNotify:SendNotification({text = _U('dive_suit_on') .. '%.'), type = "info", timeout = 5000})

			Citizen.Wait(50000)
			ESX.ShowNotification(_U('oxygen_notify', '~y~', '50') .. '%.')
			--exports.pNotify:SendNotification({text = _U('oxygen_notify', '~y~', '50') .. '%.'), type = "info", timeout = 5000})

			Citizen.Wait(25000)
			ESX.ShowNotification(_U('oxygen_notify', '~o~', '25') .. '%.')
			--exports.pNotify:SendNotification({text = _U('oxygen_notify', '~o~', '25') .. '%.'), type = "info", timeout = 5000})

			Citizen.Wait(25000)
			ESX.ShowNotification(_U('oxygen_notify', '~r~', '0') .. '%.')
			--exports.pNotify:SendNotification({text = _U('oxygen_notify', '~r~', '0') .. '%.'), type = "info", timeout = 5000})
	...
	if IsPedSittingInAnyVehicle(playerPed) then
		...
		if DoesEntityExist(vehicle) and IsPedOnFoot(playerPed) then
			...
			Citizen.CreateThread(function()
				...
				exports.pNotify:SendNotification({text = _U('used_repairkit'), type = "info", timeout = 5000})
				--ESX.ShowNotification(_U('repair_done'))
				exports.pNotify:SendNotification({text = _U('repair_done'), type = "info", timeout = 5000})
			end)
		else
			--ESX.ShowNotification(_U('error_no_veh'))
			exports.pNotify:SendNotification({text = _U('error_no_veh'), type = "info", timeout = 5000})
		end
	end
end)
-- End of Repair Kit
		...
		if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
			if DoesEntityExist(vehicle) and IsPedOnFoot(playerPed) and closestTire ~= nil then
				...
				Citizen.CreateThread(function()
					...
					exports.pNotify:SendNotification({text = _U('used_tirekit'), type = "info", timeout = 5000})
					--ESX.ShowNotification(_U('tire_done'))
					exports.pNotify:SendNotification({text = _U('tire_done'), type = "info", timeout = 5000})
				end)
			else
				--ESX.ShowNotification(_U('error_no_tire'))
				exports.pNotify:SendNotification({text = _U('error_no_tire'), type = "info", timeout = 5000})
			end
		else
			--ESX.ShowNotification(_U('error_no_veh'))
			exports.pNotify:SendNotification({text = _U('error_veh'), type = "info", timeout = 5000})
...
AddEventHandler('esx_extraitems:weabox', function()
	...
	if IsPedArmed(playerPed, 4) then
		...
		if hash ~= nil then
			...
			exports.pNotify:SendNotification({text = _U('used_weabox'), type = "info", timeout = 5000})
		else
			--ESX.ShowNotification(_U('no_weapon'))
			exports.pNotify:SendNotification({text = _U('no_weapon'), type = "info", timeout = 5000})
		end
	else
		--ESX.ShowNotification(_U('not_suitable'))
		exports.pNotify:SendNotification({text = _U('not_suitable'), type = "info", timeout = 5000})
	end
end)
-- End of Weapon Box
...
AddEventHandler('esx_extraitems:weaclip', function()
	...
	if IsPedArmed(playerPed, 4) then
		...
		if hash ~= nil then
			...
			exports.pNotify:SendNotification({text = _U('used_weaclip'), type = "info", timeout = 5000})
		else
			--ESX.ShowNotification(_U('no_weapon'))
			exports.pNotify:SendNotification({text = _U('no_weapon'), type = "info", timeout = 5000})
		end
			...
			if DoesEntityExist(playerVeh) then
				if Config.LimitedVehicles then
					...
					if IsThisModelABoat(model) or IsThisModelACar(model) or IsThisModelAHeli(model) or IsThisModelAPlane(model) or IsThisModelAnAmphibiousCar(model) then
						...
						--exports.pNotify:SendNotification({text = _U('gps_installed'), type = "info", timeout = 5000})
					else
						ESX.ShowNotification(_U('gps_not_correct'))
						--exports.pNotify:SendNotification({text = _U('gps_not_correct'), type = "info", timeout = 5000})
					end
				else
					ShowRadar = true
					ESX.ShowNotification(_U('gps_installed'))
				...
				--exports.pNotify:SendNotification({text = _U('gps_no_vehicle'), type = "info", timeout = 5000})
			end
		else
			DisplayRadar(false)
			ShowRadar = false
			ESX.ShowNotification(_U('gps_removed'))
			--exports.pNotify:SendNotification({text = _U('gps_removed'), type = "info", timeout = 5000})
		end
	end
end)
