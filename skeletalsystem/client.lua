function skeletalsystem:Start()
  while not ESX do Citizen.Wait(5); end
  while not ESX.IsPlayerLoaded() do Citizen.Wait(5); end

  self:SetupBones()
  while not self.BoneCats do Citizen.Wait(5); end
  self:SetupTextures()
...
function skeletalsystem:DamageCheck()  
  ...
  if self.plyHealth < prevHealth then    
    ESX.ShowNotification('You are injured! Check your damage by typing /skelly')
...
RegisterCommand('skelly', function(...) if not skeletalsystem.ViewingOther then skeletalsystem:HandleMenu(true); end; end)
