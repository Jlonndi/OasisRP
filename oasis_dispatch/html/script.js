...
$(document).keyup(function (e) {
  if (e.keyCode === 27) {
    ...
    $.post('https://oasis_dispatch/close', JSON.stringify({}));
...
function forwardCall(num) {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == num) {
      $.post('https://oasis_dispatch/forwardCall', JSON.stringify({
...
function copyNumber() {
  ...
  $.post('https://oasis_dispatch/copynumber', JSON.stringify({}));
}

function toggleOffduty() {

  $.post('https://oasis_dispatch/toggleoffduty', JSON.stringify({}));

}

function togglecallblips() {
  $.post('https://oasis_dispatch/togglecallblips', JSON.stringify({}));
}

function toggleUnitBlips() {
  $.post('https://oasis_dispatch/toggleunitblips', JSON.stringify({}));
}

function toggleAlerts() {

  $.post('https://oasis_dispatch/togglealerts', JSON.stringify({}));

}

function changeToBusy() {
  $.post('https://oasis_dispatch/updatestatus', JSON.stringify({
    id: id,
    status: 'Busy'
  }));
}
...
function dismissCall() {
  ...
  acceptedCall = '';
  $.post('https://oasis_dispatch/dismissCall', JSON.stringify({
    id: currentElement.id
  }));
...
function removeCall() {
  ...
  $.post('https://oasis_dispatch/removeCall', JSON.stringify({
    id: currentElement.id
  }));
...
function acceptCall() {
  ...
  $.post('https://oasis_dispatch/acceptCall', JSON.stringify({
    id: currentElement.id,
  ...
  $.post('https://oasis_dispatch/sendnotice', JSON.stringify({
...
function requestBackup() {
  ...
  for (const [key, value] of Object.entries(jobInfo)) {
    if (value.column == col) {
      $.post('https://oasis_dispatch/reqbackup', JSON.stringify({
        job: key,
