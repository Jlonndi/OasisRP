			...
			if Config.EnableESXService then
				ESX.TriggerServerCallback('esx_service:isInService', function(isInService)
					if isInService then
						...
						exports["rp-radio"]:RemovePlayerAccessToFrequencies(10, 11, 12, 13, 14, 15)
					...
					if Config.MaxInService == -1 then
						ESX.TriggerServerCallback('esx_service:enableService', function(canTakeService, maxInService, inServiceCount)
							if not canTakeService then
								...
								exports["rp-radio"]:GivePlayerAccessToFrequencies(10, 11, 12, 13, 14, 15)
						...
						if health == 0 then
							...
							local closestPlayerPed = GetPlayerPed(closestPlayer)
						  	...
						  	Citizen.CreateThread(function()
								...
								if GetEntityHealth(closestPlayerPed) == 0 then
							  		TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(closestPlayer))
							  		ESX.ShowNotification("Revive Complete" .. GetPlayerName(closestPlayer))
								else
							  		ESX.ShowNotification(GetPlayerName(closestPlayer) .. "Is dead")
								end
						  	end)
						else
						  ESX.ShowNotification(GetPlayerName(closestPlayer) .. "Unconscious")
						end --]]
					end
				else
...
function OpenBodySearchMenu(player)
	...
	--[[ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'body_search', {
	 ...
	 if data.current.value then
		 TriggerEvent('opod-evidencelocker:confiscateItems', GetPlayerServerId(player), data.current.itemType, data.current.value, data.current.amount, data.current.label)
		 menu.close()
		 Citizen.Wait(150)
...
Citizen.CreateThread(function()
	while true do
		...
		if isHandcuffed then
			...
			DisableControlAction(0, 246, true) -- Inventory
			DisableControlAction(0, 170, true) -- Animations
			DisableControlAction(0, 167, true) -- Job

			DisableControlAction(0, 11, true) -- Job
			DisableControlAction(0, 303, true) -- Job
	...
	while true do
		...
		if ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' then
			...
			for k,v in pairs(Config.PoliceStations) do
				...
				if Config.EnablePlayerManagement and ESX.PlayerData.job.grade_name == 'boss' or ESX.PlayerData.job.grade_name == 'chief' or ESX.PlayerData.job.grade_name == 'astchief' then
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, 289) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'police' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'police_actions') then
			if not Config.EnableESXService then
