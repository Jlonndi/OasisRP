...
Citizen.CreateThread(function ()
	...
	if Config.EnablePlayerManagement then
		if ESX.PlayerData.job.name == 'cardealer5' then
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'shop_confirm', {
			...
			if data2.current.value == 'yes' then
				if Config.EnablePlayerManagement then
					...
					end, 'cardealer5', vehicleData.model)
				else
			...
			ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'set_vehicle_owner_sell_amount', {
				...
				if amount == nil then
					...
					if closestPlayer == -1 or closestDistance > 3.0 then
						...
						TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_cardealer5', _U('car_dealer'), tonumber(data2.value))
					end
				end
			end, function (data2, menu2)
				menu2.close()
...
function OpenBossActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'reseller',{
		...
		if data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBossMenu', 'cardealer5', function(data, menu)
				menu.close()
			end, {wash = false})
...
AddEventHandler('esx_vehicleshop:hasEnteredMarker', function (zone)
	if zone == 'ShopEntering' then
		...
		if Config.EnablePlayerManagement then
			if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealer5' then
				CurrentAction     = 'reseller_menu'
				CurrentActionMsg  = _U('shop_menu')
				CurrentActionData = {}
			end
	...
	elseif zone == 'BossActions' and Config.EnablePlayerManagement and ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'cardealer5' and ESX.PlayerData.job.grade_name == 'boss' then
