				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					if GetVehicleClass(veh) == 18 then
						...
						if not IsPauseMenuActive() then
							...
							if IsDisabledControlJustReleased(0, 85) or IsDisabledControlJustReleased(0, 3) then
							...
							-- TOG LX SIREN
							--elseif IsDisabledControlJustReleased(0, 27) or IsDisabledControlJustReleased(0, 82) then
							elseif IsDisabledControlJustReleased(0, 27) or IsDisabledControlJustReleased(0, 3) then
							...
							elseif IsDisabledControlJustReleased(0, 3) then
								if state_pwrcall[veh] == true then
									PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
									TogPowercallStateForVeh(veh, false)
									count_bcast_timer = delay_bcast_timer
					...
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 then
						...
						if not IsPauseMenuActive() then
							...
							-- IND L
							if IsDisabledControlJustReleased(0, 69) then -- INPUT_VEH_PREV_RADIO_TRACK
								...
								count_bcast_timer = delay_bcast_timer			
							-- IND R
							elseif IsDisabledControlJustReleased(0, 70) then -- INPUT_VEH_NEXT_RADIO_TRACK
								local cstate = state_indic[veh]
								...
								count_bcast_timer = delay_bcast_timer
							-- IND H
							elseif IsControlJustReleased(0, 27) then -- INPUT_FRONTEND_CANCEL / Backspace
