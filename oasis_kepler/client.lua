...
Citizen.CreateThread(function()
	...
	Citizen.Wait(1000)
    TriggerServerEvent('oasiskepler:getVehicles')
    --GetJob()
end)
    ...
    while true do
        ...
        if (inRing) then
            ...
            if IsControlJustReleased(1, 38) then --38 = E Key
               TriggerServerEvent('oasiskepler:RingRing')
            end
        end
...
---- Client Events ----
RegisterNetEvent('oasiskepler:ShowMSG')
AddEventHandler('oasiskepler:ShowMSG', function(msg, OnlyForJob)
...
RegisterNetEvent('oasiskepler:ResultSocietyMoney')
AddEventHandler('oasiskepler:ResultSocietyMoney', function(money)
    currentSocietyMoney = money
    gotDataMoney = true
end)

RegisterNetEvent('oasiskepler:getEmployeesResult')
AddEventHandler('oasiskepler:getEmployeesResult', function(employee)
    employees = {}
    employees = employee
    gotEmployees = true
end)
...
RegisterNetEvent('oasiskepler:ShowAdvancedMSG')
AddEventHandler('oasiskepler:ShowAdvancedMSG', function(pic, title, subtitle, msg)
    ShowAdvancedNotification(pic, title, subtitle, msg)
end)

RegisterNetEvent('oasiskepler:getVehiclesResult')
AddEventHandler('oasiskepler:getVehiclesResult', function(Data)
    Vehicles = {}
    Vehicles = Data
    GetJob()
end)

RegisterNetEvent('oasiskepler:CheckTimeResult')
AddEventHandler('oasiskepler:CheckTimeResult', function(data)
    timeToLiver = data
end)
---- functions ----
...
function GenerateBossMenu()
    ...
    TriggerServerEvent('oasiskepler:getSocietyMoney')
    TriggerServerEvent('oasiskepler:getEmployeesDB')
		...
		local onScreenDisplayResult_shopmenu_amount = CreateDialog(Translation[Cfg.Locale]['how_much'])
			if (onScreenDisplayResult_shopmenu_amount ~= nil) then
				if tonumber(onScreenDisplayResult_shopmenu_amount) then
                    ...
                    TriggerServerEvent('oasiskepler:addSocietyMoney', amount)
    ...
    for j, ply in pairs(players) do
        ...
        hirePlayerItem.Activated = function(sender, index)
            TriggerServerEvent('oasiskepler:hire', GetPlayerServerId(ply), GetPlayerName(ply))
        end
    end
    ...
    for i, emp in pairs(employees) do
        ...
        for o, grade in pairs(Cfg.Grades) do
            ...
            grade_item.Activated = function(sender, item)
                TriggerServerEvent('oasiskepler:promote', emp.identifier, emp.firstname, emp.lastname, grade.number, grade.label)
            end
        end
        fire.Activated = function(sender, item)
            TriggerServerEvent('oasiskepler:fire', emp.identifier, emp.firstname, emp.lastname)
            TriggerServerEvent('oasiskepler:getEmployeesDB')
        end
    end
...
function GenerateComputerMenu(x, y, z, rot)
    ...
    if Cfg.useOneSyncInfinity then
        ESX.TriggerServerCallback('oasiskepler:getPlayersInArea', function(playersInArea_res)
                ...
                if v.storage > 0 then
                    ...
                    if tonumber(price) then
                        ...
                        if price ~= nil then
                            ...
                            -- MONEY Check
                            ESX.TriggerServerCallback('oasiskepler:hasEnoughMoney', function(hasEnoughMoney)
                                
                                if hasEnoughMoney then
                                    local veh = SpawnVehicle(v.model, SpawnLocation.x, SpawnLocation.y, SpawnLocation.z, SpawnLocation.rot, false, false, false)
                                    --print('SpawnVehicle')
                                    TriggerServerEvent('oasiskepler:removeMoney', ServerIdtargetPlayer, price)
                                    ...
                                    TriggerServerEvent('oasiskepler:BuyVeh', ServerIdtargetPlayer, v.manufacturer, v.label, vehProps, v.storage, v.model, price, NetworkGetNetworkIdFromEntity(veh), GetEntityCoords(veh), GetEntityHeading(veh))
    ...
    for k,v in pairs(Vehicles) do
        ...
        ps_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasiskepler:updateInfo', v.id, 'ps', newValue)
            end
        end
        ...
        maxSpeed_item.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasiskepler:updateInfo', v.id, 'maxSpeed', newValue)
            end
        end
        ...
        speedup.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasiskepler:updateInfo', v.id, 'speedUp', newValue)
            end
        end
        ...
        modifikation.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasiskepler:updateInfo', v.id, 'modification', newValue)
            end
        end
        ...
        tankcapacity.Activated = function(sender, index)
            ...
            if newValue then
                ...
                TriggerServerEvent('oasiskepler:updateInfo', v.id, 'fuelcapacity', newValue)
            end
        end
        ...
        if isBoss then
            ...
            liver.Activated = function(sender, item)
                ...
                if tonumber(ordertotal) then
                    ...
                    TriggerServerEvent('oasiskepler:liverVehicle', v.model, ordertotal, v.label, v.manufacturer)
                end
            end
            ...
            produce.Activated = function(sender, item)
                ...
                if tonumber(producetotal) then
                    ...
                    TriggerServerEvent('oasiskepler:produceVehicle', v.model, producetotal, v.label, v.manufacturer)
                end
            end
            if v.orderd == 1 then
        ...
        for k,v in pairs(Vehicles) do
            if v.produced == 1 then
                ...
                check.Activated = function(sender, item)
                    TriggerServerEvent('oasiskepler:CheckTime', v.time, v.lastorder, v.model, v.storage, v.producetotal)
                end
            end
        end
    end
...
RegisterCommand("Clientcoco", function(source, args, rawCommand)
    TriggerServerEvent('oasiskepler:getVehicles')
end, false)
