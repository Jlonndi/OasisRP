...
RegisterNetEvent('t1ger_scrapyard:ShowNotifyESX')
AddEventHandler('t1ger_scrapyard:ShowNotifyESX', function(msg)
	ShowNotifyESX(msg)
...
function AlertPoliceFunction()
	local pos = GetEntityCoords(GetPlayerPed(-1), false)
	--TriggerServerEvent('t1ger_scrapyard:PoliceNotifySV', GetEntityCoords(PlayerPedId()), streetName)
	exports['core_dispach']:addCall("10-16", "Stolen Car", {
		{icon="fa-car-crash", info= curVehicle},
		{icon="fa-car-crash", info= streetName},
		}, {pos.x, pos.y, pos.z}, "police", 3000, 11, 5 )
end

RegisterNetEvent('t1ger_scrapyard:PoliceNotifyCL')
AddEventHandler('t1ger_scrapyard:PoliceNotifyCL', function(alert)
	print(alert)
	if isCop then
		print("alert: "..alert)
...
RegisterNetEvent('t1ger_scrapyard:PoliceNotifyBlip')
AddEventHandler('t1ger_scrapyard:PoliceNotifyBlip', function(targetCoords)
