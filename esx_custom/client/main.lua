...
AddEventHandler('esx:playerLoaded', function(xPlayer)
	PlayerData = xPlayer
	ESX.TriggerServerCallback('esx_custom:getVehiclesPrices', function(vehicles)
		Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_custom:installMod')
AddEventHandler('esx_custom:installMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_custom:refreshOwnedVehicle', myCar)
end)

RegisterNetEvent('esx_custom:cancelInstallMod')
AddEventHandler('esx_custom:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
...
function OpenLSMenu(elems, menuName, menuTitle, parent)
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), menuName,
	{
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				...
				if data.current.label == _U('by_default') or string.match(data.current.label, _U('installed')) then
					...
					TriggerEvent('esx_custom:installMod')
				else
					...
					if isRimMod then
						...
						TriggerServerEvent("esx_custom:buyMod", price)
		...
		menu.close()
		TriggerEvent('esx_custom:cancelInstallMod')
