	...
	ESX.TriggerServerCallback('t1ger_shops:getItemStock', function(stockSV)
		...
		for k,v in pairs(Config.Items) do
			...
			if typeChecked then 
				...
				local string_match = nil; if v.str_match ~= nil then string_match = v.str_match end
				if #stockSV > 0 then
					...
					table.insert(elements, {
						...
						name = v.label, item = order_item, price = v.price, count = currentCount, str_match = string_match
					})
				else
					table.insert(elements, {
						label = v.label..'<span style="color:MediumSeaGreen;"> [ '..'<span style="color:GoldenRod;">'..currentCount.."x pcs </span> ]",
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'order_stock_confirmation', {
						...
						if data3.current.value == 'button_yes' then
							...
							ESX.TriggerServerCallback('t1ger_shops:payStockOrder', function(paidOrder)
								if paidOrder then
									TriggerServerEvent('t1ger_shops:sendStockOrder', id, selected.name, selected.item, selected.count, selected.str_match, data2.current.value, selected.price, order_price, shelf.type)
								else
