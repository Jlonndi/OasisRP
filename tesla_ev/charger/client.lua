...
local function chargingLoop(ped, vehicle, vcoords, cscoords)
    ...
    if level >= 100.0 then
        SIThreads:LoopForDuration('SS_EV_TEXT_ALREADY_CHARGED', 3000, function()
            Citizen.Wait(1)
            DrawText3Ds(cscoords.x, cscoords.y, cscoords.z + 1.0, "Your EV is already charged.")
        end)
        return
    end
    ...
    while true do
        ...
        DrawText3Ds(cscoords.x, cscoords.y, cscoords.z + 1.0, "Press ~g~G ~w~to stop charging your EV.")
...
local function runChargingStation(ped, cscoords, stop)
    ...
    if IsPedInAnyEVVehicle(ped) then
        ...
        AddTextComponentString('Exit your EV to charge.')
    ...
    if vdistance < 3 then
        SetTextComponentFormat('STRING')
        AddTextComponentString('Press ~INPUT_PICKUP~ to start charging your EV')
        DisplayHelpTextFromStringLabel(0, 0, 1, -1)
        ...
        SetTextComponentFormat('STRING')
        AddTextComponentString('Your EV is too far away from the charger.')
        DisplayHelpTextFromStringLabel(0, 0, 1, -1)
    end
end
